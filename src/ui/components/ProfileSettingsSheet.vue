...
<template>
  ...
  >
    ...
    >
      <div
        aria-label="设置遮罩"
        class="sheet-backdrop absolute inset-0 bg-black/45 backdrop-blur-[2px]"
        @click="emit('close')"
      />
      <div
        aria-label="设置弹层"
        role="dialog"
        aria-modal="true"
        class="sheet-panel relative w-full max-w-2xl rounded-t-2xl border border-border-paper bg-paper-card p-4 shadow-[0_-8px_24px_rgba(34,26,16,.18)] sm:max-w-md sm:rounded-2xl sm:shadow-[0_16px_48px_rgba(0,0,0,.35)]"
      >
